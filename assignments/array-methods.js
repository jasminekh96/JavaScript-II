...
// Problem 1
let LastNamesAllUpper = runners.map(function(runner) {
	return runner.last_name.toUpperCase();
});
console.log(LastNamesAllUpper);
// Problem 2
let runnersSmallSizeShirt = runners.filter(function(runner) {
...
let NameAndState = [];
runners.forEach(function(names) {
	return NameAndState.push(`${names.first_name} ${names.company_name}`);
});
